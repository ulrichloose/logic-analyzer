//The pde frontend with some improvements...
//
//you can choose now the serial port and need not to hardcode the name.
//Furthermore the timinigs are shown in uS for one edge to the next one and the captured data ist initially always displayed
//left justified.
    ...
    for (int i=0; i<samples; i++) {
      ...
      for (int n=0; n<6; n++)
      {
        if (state[i][n]==true)
        {
          ...
          if (drawTimes == true)
          {
            ...
            line(xTime[i]+xShift, yPos, xTime[i]+xShift, (textCovered==true) ? yBottom : yBottom-15);
            stroke(green);
          }
      ...
      // Text times
          if (drawTimes == true)
          {
            ...
            text(round(usTime[(i+1) % samples] - usTime[i]), (xTime[i])+xShift-4, (textCovered==true) ? yBottom : yBottom+15);    //write on different height
            stroke(green);
          }
    }
